...
Graph.prototype.getColorStatus = function() {
  ...
  if (Engine.gameState === "IN_PROGRESS") {
    return Engine.currentlyPlaying() ? themeStatus.PLAYING : themeStatus.NOT_PLAY;
  }
...
Graph.prototype.getCurrentGamePayout = function() {
  return Clib.calcGamePayout(Clib.getElapsedTimeWithLag(Engine));
}
...
Graph.prototype.drawGameData = function () {
  ...
  if (Engine.gameState === 'STARTING') {
    ...
    const text = timeLeft >= 0 ? ('Next round in ' + timeLeft + 's') : 'Closed. Round are starting ...';
